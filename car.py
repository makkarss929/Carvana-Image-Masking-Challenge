...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 car.py train --dataset=./datasets --weights=coco
    # Resume training a model that you had trained earlier
    python3 car.py train --dataset=./datasets --weights=../../logs/car20180815T0023/mask_rcnn_car_0067.h5
    # Train a new model starting from ImageNet weights
    python3 car.py train --dataset=./datasets --weights=imagenet
...
class carDataset(utils.Dataset):
    ...
    def load_car(self, dataset_dir, subset):
        ...
        car_segmentations_df = pd.read_csv(os.path.join(dataset_dir, "{}_masks.csv".format(subset)))
        car_segmentations_df = car_segmentations_df.sample(frac=0.05)
        image_ids = car_segmentations_df.ImageId.unique()

        for image_id in image_ids:

            img_masks = car_segmentations_df.loc[car_segmentations_df['ImageId'] == image_id, 'rle_mask'].tolist()

            image_path = os.path.join(dataset_dir,subset,image_id)
    ...
    def test_endcode_decode(self):
        ...
        car_DIR = os.path.join(ROOT_DIR, "datasets")
        car_segmentations_df = pd.read_csv(os.path.join(car_DIR,"train_masks.csv"))
        rle_img_masks = car_segmentations_df.loc[car_segmentations_df['ImageId'] == "0005d01c8.jpg", 'rle_mask']
    ...
    def masks_as_image(self,in_mask_list):
        # Take the individual car masks and create a single mask array for all cars
...
def train(model):
    ...
    #dataset_train.load_car('./datasets', "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = carDataset()
    dataset_val.load_car(args.dataset, "val")
    #dataset_val.load_car('./datasets', "val")
    dataset_val.prepare()
    ...
    #mc = keras.callbacks.ModelCheckpoint('best_model_weights.hdf5', monitor='val_loss', mode='min', verbose=1,save_best_only=True)
    model.summary()
